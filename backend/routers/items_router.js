...
itemsRouter.post("/", upload.single("file"), async (req, res) => {
    ...
    if (!room) {
        return res.status(404).json({ error: `Room(id=${req.query.roomId}) not found.` });
    }
...
itemsRouter.get("/:id", async (req, res) => {
    ...
    if (!item) {
        return res.status(404).json({ error: `Item(id=${req.params.itemId}) not found.` });
    }
    return res.json(item);
    });
...
itemsRouter.get("/catergories/:type", async (req, res) => {
    ...
    if (items.length === 0) {
        return res.status(404).json({ error: `Items with category ${req.params.type} not found.` });
    }
